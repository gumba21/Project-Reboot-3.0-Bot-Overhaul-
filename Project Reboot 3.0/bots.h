...
namespace Bots
{
	...
	inline bool ForceKillBotForStressTest(uint64 BotId, AController* KillerController)
	{
		...
		EnterLifecycleDiagnosticStage(BotId, EBotLifecycleDiagnosticStage::DamageApplication);
		LOG_INFO(LogBots, "[BotStress] Damage application entry bot={} callback=FortPawn.ForceKill.", BotId);
		Pawn->ProcessEvent(ForceKillFn, &ForceKillParams);
		LOG_INFO(LogBots, "[BotStress] Damage application exit bot={} callback=FortPawn.ForceKill.", BotId);
		CompleteLifecycleDiagnosticStage(BotId, EBotLifecycleDiagnosticStage::DamageApplication);
		return true;
	}
	...
	{
		if (!ShouldRunInvalidBotSweep(bFromNetworkTick))
			return;

		const auto StressSnapshot = GetBotStressDiagnosticSnapshot();
		const bool bLogDiagnosticStage = StressSnapshot.bActive;

		if (bLogDiagnosticStage)
		{
			EnterLifecycleDiagnosticStage(0, EBotLifecycleDiagnosticStage::InvalidBotSweep);
			LOG_INFO(LogBots, "[BotLifecycle] Invalid-bot sweep entry source={}.",
				bFromNetworkTick ? "network-tick" : "explicit");
		}
		...
		if (bLogDiagnosticStage)
		{
			LOG_INFO(LogBots, "[BotLifecycle] Invalid-bot sweep exit source={} invalidCount={}.",
				bFromNetworkTick ? "network-tick" : "explicit", InvalidBotIds.size());
			CompleteLifecycleDiagnosticStage(0, EBotLifecycleDiagnosticStage::InvalidBotSweep);
		}
	}
	...
	inline void HandleMatchReset(const char* Reason)
	{
		...
		ResetBotStressDiagnostics();
		const int RemovedCount = DespawnAllBots(Reason);
		GetRegistry().ClearRetiredControllerTombstones(Reason);
	...
	inline void Shutdown(bool bDestroyActors)
	{
		...
		GetRegistry().ClearRetiredControllerTombstones("bot-system shutdown", false);
	}
